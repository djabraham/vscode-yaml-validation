...
class State{
    ...
    lineStart:number
    lineIndent:number
    documents:YamlInterface.YAMLNode[];
    kind:string
    result:YamlInterface.YAMLNode
    tag:string
    anchor:string
    anchorMap:{ [name:string]:YamlInterface.YAMLNode}
    tagMap:any
    version:string
...
function captureSegment(state:State, start:number, end:number, checkJson:boolean):void {
  ...
  var scalar:YamlInterface.YAMLScalar=<YamlInterface.YAMLScalar>state.result;
  if (scalar.start==-1){
      scalar.start=start;
  }
  if (start < end) {
...
function storeMappingPair(state:State, _result:YamlInterface.YamlMap, keyTag, keyNode:YamlInterface.YAMLNode, valueNode:YamlInterface.YAMLNode, parent?: any):YamlInterface.YamlMap {
  var index, quantity;
  if (keyNode==null){
      return;
  }
  ...
  if ('tag:yaml.org,2002:merge' === keyTag) {
    ...
    var mapping = YamlInterface.newMapping(<YamlInterface.YAMLScalar>keyNode, valueNode);
...
function writeFoldedLines(state:State, scalar, count:number) {
  if (1 === count) {
    scalar.buffer += ' ';
  } else if (count > 1) {
...
function readFlowCollection(state:State, nodeIndent) {
  var readNext = true,
      _line,
      _tag     = state.tag,
      _result:YamlInterface.YAMLNode,
  ...
  while (0 !== ch) {
    ...
    if (isMapping) {
      storeMappingPair(state, (<YamlInterface.YamlMap>_result), keyTag, keyNode, valueNode);
    } else if (isPair) {
        var mp=storeMappingPair(state, null, keyTag, keyNode, valueNode, _result);
        mp.parent=_result;
        (<YamlInterface.YAMLSequence>_result).items.push(mp);
    } else {
        keyNode.parent=_result;
        _result.kind = YamlInterface.Kind.SEQ;
        (<YamlInterface.YAMLSequence>_result).items.push(keyNode);
    }
  ...
  if (null !== state.tag && '!' !== state.tag) {
    if (state.tag=="!include"){
      ...
      for (typeIndex = 0, typeQuantity = state.implicitTypes.length;
        ...
        if (!vl) console.log("NULL: ", state.result);
...
function exportResult(state: State) {
